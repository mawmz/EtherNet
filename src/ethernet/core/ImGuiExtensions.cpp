...
namespace ethernet::core::imguiext {
	bool MenuItemToggle(const char* label, bool* selected) {
		...
		const float checkmarkWidth = IM_FLOOR(context.FontSize * 1.20f);
		const float minimumWidth = window->DC.MenuColumns.DeclColumns(
			0.f,
			labelSize.x,
			0.f,
			checkmarkWidth
		);
		const float stretchWidth = ImMax(
			0.f,
			ImGui::GetContentRegionAvail().x - minimumWidth
		);
		const float markOffset = window->DC.MenuColumns.OffsetMark;

		const bool pressed = ImGui::MenuItem(label, nullptr, false);
		if(pressed)
			*selected = !*selected;
		if(
			*selected
			&& (context.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) != 0
		) {
			ImGui::RenderCheckMark(
				window->DrawList,
				ImVec2(
					position.x + markOffset + stretchWidth
						+ context.FontSize * 0.40f,
					position.y + context.FontSize * 0.134f * 0.5f
				),

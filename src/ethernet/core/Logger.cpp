...
namespace ethernet::core {
	...
	void Logger::VLogMessage(Severity severity, fmt::string_view format, fmt::format_args args) {
		...
		skylaunch::logger::s_Instance->LogFormat("[EtherNet|%s] %s", fmt::format("{}", severity).c_str(), formatted.c_str());

		NN_DIAG_LOG(nn::diag::LogSeverity::Info, "[EtherNet|%s] %s", fmt::format("{}", severity).c_str(), formatted.c_str());
	...
	void Logger::VToastMessage(std::string_view group, Severity severity, fmt::string_view format, fmt::format_args args) {
		...
		// by god I wish I didn't have to deal with this
		skylaunch::logger::s_Instance->LogFormat("[EtherNet|%s~%s] %s", fmt::format("{}", severity).c_str(), group.data(), formatted.c_str());

		NN_DIAG_LOG(nn::diag::LogSeverity::Info, "[EtherNet|%s~%s] %s", fmt::format("{}", severity).c_str(), group.data(), formatted.c_str());
	...
	void Logger::DrawToasts(fw::UpdateInfo* updateInfo) {
		...
		for(std::size_t i = 0; i < toastLines.size(); ++i) {
			...
			if(msg.lifetime > 0) {
				...
				int width = ethernet::core::debug::drawFontFmtGetWidth("{}", msg.text);
	...
	void Logger::DrawInternal(LoggerMessage& msg, std::uint16_t x, std::uint16_t y, bool showSeverity) const {
		...
		if(showSeverity)
			ethernet::core::debug::drawFontFmtShadow(x, y, colMain, "[{}] {}", msg.severity, msg.text);
		else
			ethernet::core::debug::drawFontFmtShadow(x, y, colMain, "{}", msg.text);
	}

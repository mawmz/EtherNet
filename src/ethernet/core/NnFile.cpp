...
namespace ethernet::core {
	...
	void NnFile::CleanPath(std::string& path, bool flat/* = false*/) {
		...
		if(flat)
			ethernet::core::StringReplace(path, "/", "_");
		ethernet::core::StringReplace(path, ":", "_");
		ethernet::core::StringReplace(path, "//", "/");
	}

	bool NnFile::EnsurePath(std::string_view& path, bool createPaths/* = true*/) {
		auto splits = ethernet::core::StringSplit(path.data(), "/");
				...
				if(R_FAILED(res)) {
					if(res == 0x202 && createPaths) { // Path does not exist
						...
						if(R_FAILED(res) && res != 0x402) {
							ethernet::core::g_Logger->LogError("Failed to create dump directory \"{}\" for reason {}", ss.str(), res);
							return false;
						}
					} else {
						ethernet::core::g_Logger->LogError("Failed to open dump directory \"{}\" for reason {}", ss.str(), res);
						return false;
					}
				} else {

...
namespace ethernet::core {
	...
	void MenuLog::Add(const Logger::Severity& severity, const std::string& text) {
		for (std::string str : ethernet::core::StringSplit(fmt::format("[{}] {}", severity, text).data(), "\n")) {
	...
	void MenuLog::Add(const Logger::Severity& severity, const std::string_view& group, const std::string& text) {
		for (std::string str : ethernet::core::StringSplit(fmt::format("[{}~{}] {}", severity, group.data(), text).data(), "\n")) {
			buffer.append(str.data());
			lineOffsets.push_back(buffer.size());
			lineSeverity.push_back(severity);
		}
	...
	void MenuLog::SaveToFile() {
		const std::string logpath = ETHERNET_CONFIG_PATH "/logs/";
		...
		ethernet::core::g_Logger->LogInfo("Attempting save at {}", timestamp);
		std::string path = logpath + timestamp + ".log";

		int size = buffer.size() + lineOffsets.size();

		if(!ethernet::core::NnFile::Preallocate(path, size)) {
			ethernet::core::g_Logger->LogError("Couldn't create/preallocate dump file \"{}\"", path);
			return;
		}

		ethernet::core::NnFile file(path, nn::fs::OpenMode_Write);

		if(!file) {
			ethernet::core::g_Logger->LogError("Couldn't save log to file \"{}\"", path);
		} else {
	...
	void MenuLog::Draw(bool* open) {
		...
		if(!ImGui::Begin("EtherNet Log", open)) {
			ImGui::End();
			return;
		}
		...
		if(ImGui::BeginChild("scrolling", ImVec2(0, 0), false, ImGuiWindowFlags_HorizontalScrollbar)) {
			...
			const char* buf_end = buffer.end();
			{
				...
				for(int line_no = 0; line_no < lineOffsets.Size; line_no++) {
					...
					if(lineSeverity[sev] < ethernet::core::g_Logger->GetLoggingLevel())
						continue;

					mm::Col4 col = ethernet::core::Logger::ColorForSeverity(lineSeverity[sev]);

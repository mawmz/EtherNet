...
namespace mm {
	...
	ETHERNET_CONVERTTO_TYPE(Vec2, glm::vec2, sizeof(glm::vec2));

	struct Vec3 : public ethernet::core::ConvertTo<glm::vec3, sizeof(glm::vec3)> {
		// We actually want to add members to Vec3 so
		//ETHERNET_CONVERTTO_TYPE(Vec3, glm::vec3, sizeof(glm::vec3));
	...
	ETHERNET_CONVERTTO_TYPE(Quat, glm::quat, sizeof(glm::quat));

	struct alignas(0x10) Mat44 : public ethernet::core::ConvertTo<glm::mat4, sizeof(glm::mat4)> {
		// We actually want to add members to Mat44 so
		//ETHERNET_CONVERTTO_TYPE(alignas(0x10) Mat44, glm::mat4, sizeof(glm::mat4));
		// won't work. Ugly using, but oh well.

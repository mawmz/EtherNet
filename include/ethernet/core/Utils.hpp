...
#define ETHERNET_DISALLOW_MOVE(T, reason) T(T&&) = delete;
#define ETHERNET_DISALLOW_COPY(T, reason) T(const T&) = delete;
...
#define ETHERNET_CODENAME(cn) defined __ETHERNET_CODENAME_##cn
#define ETHERNET_CODENAME_STR _ETHERNET_CODENAME_STR
...
#define ETHERNET_OLD_ENGINE (ETHERNET_CODENAME(bf2) || ETHERNET_CODENAME(ira) || ETHERNET_CODENAME(bf2_ira))
#define ETHERNET_NEW_ENGINE (ETHERNET_CODENAME(bfsw) || ETHERNET_CODENAME(bf3))
...
namespace ethernet::core {
	...
	namespace detail {
		...
		constexpr auto operator |(T lhs, T rhs) requires(std::is_enum_v<std::remove_cvref_t<T>>) {
			return (ethernet::core::underlying_value(lhs) | ethernet::core::underlying_value(rhs));
		}
		...
		constexpr auto operator ~(T lhs) requires(std::is_enum_v<std::remove_cvref_t<T>>)  {
			return ~ethernet::core::underlying_value(lhs);
		}

		template<typename T>
		constexpr bool bitMask(T lhs, T mask) {
			return (lhs & mask) == ethernet::core::underlying_value(mask);
		}

		template<typename T>
		constexpr bool bitMaskStrict(T lhs, T mask) {
			return ethernet::core::underlying_value(lhs) == ethernet::core::underlying_value(mask);
		}
	}
	...
	using ethernet::core::detail::bitMask;
	using ethernet::core::detail::bitMaskStrict;

#define ETHERNET_CONVERTTO_TYPE(T, U, size) \
    struct T : public ethernet::core::ConvertTo<U, size> { \
...
using ethernet::core::detail::operator|;
using ethernet::core::detail::operator&;
using ethernet::core::detail::operator^;
using ethernet::core::detail::operator~;

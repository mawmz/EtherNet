...
namespace ethernet::core {
	...
	struct ModuleRegistrar {
		...
		explicit ModuleRegistrar(const char* name, TUpdatable* updatable) {
			if(std::string_view(name).starts_with("ethernet::core::"))
				detail::RegisterModule(name + std::string_view("ethernet::core::").size(), updatable);
			else
				detail::RegisterModule(name, updatable);
		}
	};

#define ETHERNET_REGISTER_MODULE(T) \
	static T module__##T;          \
	::ethernet::core::ModuleRegistrar<T> register__##T(#T, &module__##T);

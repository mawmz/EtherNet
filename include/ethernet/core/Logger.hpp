...
namespace ethernet::core {
	...
	struct Logger {
		...
		ETHERNET_DISALLOW_COPY(Logger, "only one instance of the logger is allowed");
		ETHERNET_DISALLOW_MOVE(Logger, "only one instance of the logger is allowed");
		...
		/**
		 * Globally suppresses ethernet::core log messages and toasts.
		 */
...
template<>
struct fmt::formatter<ethernet::core::Logger::Severity> : fmt::formatter<std::string_view> {
	template<typename FormatContext>
	inline auto format(ethernet::core::Logger::Severity sev, FormatContext& ctx) {
		using enum ethernet::core::Logger::Severity;
